...
# Biuld the Geometry, Point, Line, Polygon firstly
# Reference to the code in the lecture slides
# add category when defining Point class, and get_category and set_category methods
# category means kind of point
...
# actually python has a class list itself and min() and max() can be used directly
# considering the methods to get min and max value were taught in lecture
...
class Classifier():
    ...
    # MBR Algorithm
    # min_bounding_rectangle_test method requires input_points_list (input Point list) and polygon
    # output a list of Point (class)
    ...
    def min_bounding_rectangle_test(self):
        ...
        # to get two lists for x, y coordinates of points of polygon
        ...
        # get the x, y coordinates of corner points of the minimum bounding polygon
        min_x = list_polygon_x.minimum()
        min_y = list_polygon_y.minimum()
        max_x = list_polygon_x.maximum()
        max_y = list_polygon_y.maximum()

        # use a for loop to check each point in input_points_list
        # simply by < > to check if the point is outside or inside in the MB rectangle
        # use if, else to set the new category of each point in the original list
    ...
    # Point on Line test
    # to categorize the points which are on the boundary of the polygon
    def point_on_line_test(self, line, point):
        ...
        # if the point is on the line, it will return True
    ...
    def use_point_on_line_test(self):
        ...
        # two for loops are used here
        # the first one is to check each point in the list
        # the second one is to test the point from the first loop with all the lines in Polygon Line List
        for i in self.__input_points_list:
    ...
    def ray_casting_test(self, line, point):
        ...
        # the parity of count is used to determine the category of points
        count = 0
        # firstly considering the ray doesn't cross the points of polygon
        if y3 != y1 and y3 != y2:

            if y1 == y2:
                if y3 == y1:
                    # the ray crosses the boundary
                    # because the points on boundary have been gotten by point_on_line test
                    # for other points, its situation will not be changed by crossing the boudary
                    if x3 <= x1 or x3 <= x2:
                        count = count
                    else:
                        count = count
...
# the method to read input points
    def read_input(self):
        input_id = []
        input_x_coordinates = []
        input_y_coordinates = []
...
def main():
    ...
    input_read = Reader("Input Reader")

    input_points_list = input_read.read_input()
